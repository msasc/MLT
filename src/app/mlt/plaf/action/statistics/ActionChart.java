...
public class ActionChart extends ActionStatistics {
	...
	private List<PlotData> getPlotDataList() {
		...
		/* Prices and averages. */
		{
			...
			info.addOutput("Open", "O", OHLC.OPEN, "Open data value", plotterPrices);
			info.addOutput("High", "H", OHLC.HIGH, "High data value", plotterPrices);
			...
			for (int i = 0; i < fields.size(); i++) {
				...
				info.addOutput(name, name, index, label, linePlotter);
			}
	...
	private PlotData getPlotDataFieldList(
		...
		for (Field field : fields) {
			...
			dataList.addPlotter(plotter);
			info.addOutput(name, name, index, label, plotter);
		}
