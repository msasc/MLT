	...
	public Relation.Type getType() {
		...
		if (check) {
			...
			if (localMatch == TOTAL_MATCH && foreignMatch == TOTAL_MATCH) {
				...
				if (localMatch == TOTAL_MATCH && foreignMatch == PARTIAL_MATCH_SEG) {
					type = Type.FOREIGN_DETAIL;
				} else {
					if (localMatch == PARTIAL_MATCH_KEY && foreignMatch == TOTAL_MATCH) {
						type = Type.LOCAL_DETAIL;
					} else {
						if (localMatch == PARTIAL_MATCH_SEG && foreignMatch == TOTAL_MATCH) {
							type = Type.LOCAL_DETAIL;
						} else {
							if (localMatch == NO_MATCH && foreignMatch == TOTAL_MATCH) {
								type = Type.LOOKUP;
							}
						}
					}
