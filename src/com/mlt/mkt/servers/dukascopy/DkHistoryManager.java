...
import com.mlt.mkt.server.DataIterator;
...
public class DkHistoryManager extends HistoryManager {
	...
	public Data getData(Instrument instrument, Period period, OfferSide offerSide, int shift) throws ServerException {
		try {
			server.ensureSubscribed(instrument);
			return DkCore.fromDkBar(getHistory().getBar(DkCore.toDkInstrument(instrument), DkCore.toDkPeriod(period),
	 ...
	 */
	@Override
	public List<Data> getDataList(Instrument instrument, Period period, OfferSide offerSide, Filter filter, long from,
	long to) throws ServerException {

		server.ensureSubscribed(instrument);
		List<Data> dataList = new ArrayList<>();
		try {
			List<IBar> bars = getHistory().getBars(DkCore.toDkInstrument(instrument), DkCore.toDkPeriod(period),
			DkCore.toDkOfferSide(offerSide), DkCore.toDkFilter(filter), from, to);
	 ...
	 */
	@Override
	public DataIterator getDataIterator(Instrument instrument, Period period, OfferSide offerSide, Filter filter,
	long from) throws ServerException {
	...
	@Override
	public DataIterator getDataIterator(Instrument instrument, Period period, OfferSide offerSide, Filter filter,
	long from, long to) throws ServerException {
		server.ensureSubscribed(instrument);
