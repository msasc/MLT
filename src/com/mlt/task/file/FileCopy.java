...
public class FileCopy extends Task {
	...
	class CountListener implements FileScannerListener {
		...
		public void file(File sourceDirectory, File file) throws IOException {
			...
			updateStatusLabel(STATUS_FROM, LABEL_FROM, file.toString());
	...
	class CopyListener implements FileScannerListener {
		...
		public void file(File sourceDirectory, File file) throws IOException {
			...
			if (file.isFile()) {
				...
				String from = Resources.getText("tokenFrom") + ": " + file.getAbsolutePath();
				String to = Resources.getText("tokenTo") + ": " + destinationFile.getAbsolutePath();
				updateStatusLabel(STATUS_FROM, LABEL_FROM, from);
				updateStatusLabel(STATUS_TO, LABEL_TO, to);
				...
				if (destinationFile.exists()) {
					if (!destinationFile.canWrite()) {
						if (!destinationFile.delete()) {
							...
							msgExc.append(Resources.getText("securityAccessToFileDenied"));
							msgExc.append(" -> ");
							msgExc.append(file);
	...
	class PurgeListener implements FileScannerListener {
		...
		public void file(File destinationDirectory, File file) throws IOException {
			...
			if (file.isFile()) {
				String purge = Resources.getText("tokenPurge") + ": " + file.getAbsolutePath();
				updateStatusLabel(STATUS_FROM, LABEL_FROM, purge);
			...
			while (!deque.isEmpty()) {
				if (Files.isEmpty(deque.getLast())) {
					...
					String purge = Resources.getText("tokenPurge") + ": " + last.getAbsolutePath();
					updateStatusLabel(STATUS_FROM, LABEL_FROM, purge);
					last.delete();
				} else {
					break;
	...
	private static final String LABEL_TO = "LABEL-TO";
	/** Additional status to show the from file. */
	private static final String STATUS_FROM = "STATUS-FROM";
	/** Additional status to show the to file. */
	private static final String STATUS_TO = "STATUS-TO";
	 ...
	 */
	@Override
	protected long calculateTotalWork() throws Exception {
	...
	private void clearMessages() {
		updateStatusLabel(STATUS_FROM, LABEL_FROM, "");
		updateStatusLabel(STATUS_TO, LABEL_TO, "");
	}
